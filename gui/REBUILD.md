...
> Logic and backend come *after* you already have a working visual shell.  
> You write every line. The `main/` folder is your reference — read it, don't copy it.

---

## Phase 0 — Foundation
*Teaches: Next.js App Router, TypeScript strict mode, Vitest setup*
...
*Teaches: Tailwind v4 `@theme`, design tokens, responsive layout, dark mode*
...
*Teaches: React components, props, composition, basic hooks, conditional rendering*

### What you build
Every major UI component the app needs — wired together with hardcoded dummy data. No state management, no API calls. Just components that *look* right.
...
- You pass data *down* via props and events *up* via callback props (`onSubmit`, `onChange`).
...
*Teaches: HTML5 Canvas 2D API, coordinate system, draw calls, easing, `requestAnimationFrame`*
...
export const easing = {
  ...
  bounce: (t: number) => { /* standard bounce formula */ },
};
```
...
*Teaches: Zustand v5, global state, localStorage persistence, optimistic UI*
...
- **Optimistic UI** = update the UI *immediately* when the user acts, then correct it if the server disagrees. Feels instant.
...
*Teaches: Zod, TypeScript type inference, TDD red→green cycle*

### What you build
Zod schemas for every data shape the app uses. These become the single source of truth for types.

### Concepts first
- Zod defines the *shape* of your data and validates it at runtime. `z.string()`, `z.number()`, `z.object({})`.
...
*Teaches: Constructing valid typed objects programmatically, using schemas as contracts*
...
```ts
export function createSeedProject(name: string, duration: SupportedDuration): VideoProject {
  return {
...
it("passes schema validation", () => {
  expect(() => VideoProjectSchema.parse(createSeedProject("test", 5))).not.toThrow();
  expect(() => VideoProjectSchema.parse(createSeedProject("test", 30))).not.toThrow();
});
```
...
*Teaches: Next.js route handlers, HTTP methods, Zod at the API boundary, stub-first development*
...
- Validate the request body with Zod *before* using any of its values. Return a 400 if it fails.
...
*Teaches: MediaRecorder API, `canvas.captureStream()`, Blob, file download*
...
*Teaches: Fetch API, OpenRouter structured JSON output, prompt engineering, pipeline design*
...
```ts
export function buildSystemPrompt(): string { /* role, rules, schema description */ }
export function buildInitialPrompt(prompt: string, duration: number, style: string): string
export function buildModifyPrompt(prompt: string, previousProject: VideoProject): string
export function buildRepairPrompt(project: VideoProject, issues: QualityIssue[]): string
```
...
*Teaches: Pure function TDD, scoring logic, systematic validation, surfacing errors in UI*
...
- TDD is ideal here because every check is deterministic: given *this* project, always return *these* issues.
- The quality gate runs *inside the AI pipeline* as a safeguard, and is also shown to the user in the UI.
