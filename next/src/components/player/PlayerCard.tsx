...
function PlayerCardFrame({
  ...
  const handleVisualCheck = async () => {
    ...
    const resolvedTiming = resolveActTimings(baseTiming, duration, message?.brief?.actWeights);
      ...
      <div className={`border-b border-border/80 px-5 py-4 ${isFullscreen ? "bg-zinc-900/50 rounded-xl mb-4" : ""}`}>
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            {/* Visual LLM Check button hidden per request */}
...
function VisualCheckReport({
  ...
  if (!visualCheck || !visualCheck.frames || visualCheck.frames.length === 0) return null;

  const currentFrame = visualCheck.frames[selectedActIndex] || visualCheck.frames[0];
    ...
    <div className="border-t border-border/80 px-5 py-4 bg-surface-raised/30 flex flex-col gap-4 text-xs">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-0.5 text-[11px]">{visualCheck.summary}</p>
        </div>
      ...
      <div className="p-3 rounded-xl border border-border/80 bg-background/50 space-y-2">
        <div className="flex items-center justify-between">
          <span className="font-bold text-foreground/90">Act {currentFrame.actIndex} Details ({currentFrame.timestamp.toFixed(1)}s)</span>
          <span className="font-mono text-muted-foreground">Frame Score: {currentFrame.score}/100</span>
        </div>
          ...
          <div className="space-y-1.5 pt-1">
            {currentFrame.issues.map((issue: any, idx: number) => (
              <div key={idx} className="flex items-start gap-2 text-[11px]">
                <span className={`mt-0.5 shrink-0 px-1 py-0.25 rounded text-[8px] font-bold uppercase tracking-wider ${
      ...
      {/* Recommendations */}
      {visualCheck.recommendations && visualCheck.recommendations.length > 0 && (
        <div className="space-y-1.5">
          <span className="font-bold text-foreground/80">Recommendations</span>
