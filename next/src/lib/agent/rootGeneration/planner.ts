...
export const VideoSceneSchema = z.object({
  id: z.string().regex(/^[a-z0-9][a-z0-9-]*$/).min(1).max(24),
...
export const VideoPlanSchema = z.object({
  ...
  scenes: z.array(VideoSceneSchema).min(2).max(5),
}).strict();
...
/** Scene-count ceiling per total duration so windows stay watchable. */
function maxScenesForDuration(duration: SupportedDuration): number {
...
export function planSceneWindows(
  ...
  const scenes = plan.scenes.slice(0, maxScenesForDuration(duration));
...
export function defaultVideoPlan(prompt: string): VideoPlan {
    ...
    scenes: [
      {
        ...
        role: "overview",
        name: "Overview",
        goal: `Introduce the essential structure of: ${normalized}`.slice(0, 300),
        share: 0.3,
      },
      {
        id: "details",
...
export async function planVideoScenes(
  ...
  const parsePlan = (raw: unknown): VideoPlan => normalizeSceneIds(VideoPlanSchema.parse(raw));
  ...
  try {
    ...
    if (!isRepairableModelOutputFailure(firstCallError)) {
      ...
      return defaultVideoPlan(request.prompt);
    }
    try {
